...
{
    //标题栏设置背景图片
    UIImage *headBgImg = [[UIImage imageNamed:@"head_nav_bg_1"] resizableImageWithCapInsets:UIEdgeInsetsMake(2, 2, 2, 2)];
    
    navigationBar.barStyle = UIBarStyleBlack;
    navigationBar.tintColor = [UIColor whiteColor]; // 返回按钮的颜色
    // 设置标题的字体属性
    navigationBar.titleTextAttributes = [NSDictionary dictionaryWithObjectsAndKeys:[UIFont boldSystemFontOfSize:19], NSFontAttributeName,
                                         [UIColor colorWithString:@"#ffffff"], NSForegroundColorAttributeName, nil];
...
-(void) setNavigationBarStyle2:(UINavigationBar *)navigationBar {
    ...
    navigationBar.barStyle = UIBarStyleBlack;
    navigationBar.tintColor = [UIColor colorWithString:@"#333333"];
    navigationBar.titleTextAttributes = [NSDictionary dictionaryWithObjectsAndKeys:[UIFont boldSystemFontOfSize:19], NSFontAttributeName,
                                         [UIColor colorWithString:@"#333333"], NSForegroundColorAttributeName, nil];
    [navigationBar setBackgroundImage:headBgImg forBarMetrics:UIBarMetricsDefault];
